...
export default function Commit({ on }: { on: boolean }) {
  ...
  useEffect(() => {
    ...
    if (matchMedia("(prefers-reduced-motion: reduce)").matches) {
      const frame = requestAnimationFrame(() => setMw(BEST));
  ...
  const pick = (clientX: number) => {
    ...
    setMw(clamp((clientX - r.left) / r.width) * MAX);
  };
  ...
  const curve =
    "M" +
    Array.from({ length: 97 }, (_, i) => `${i * 10},${(78 - risk(i) * 76).toFixed(2)}`).join(" L");
  const state = mw < 30 ? "low" : r > 0.15 ? "high" : "right";
  const verdict =
    state === "low"
      ? `${Math.round(BEST - mw)} MW left unallocated`
      : state === "high"
        ? "one missed event revokes it all"
  ...
  return (
    <div className={`commit-ui${on ? " on" : ""}`} data-state={state}>
      ...
      <div
        ...
        aria-valuemin={0}
        aria-valuemax={MAX}
        aria-valuenow={Math.round(mw)}
        aria-label="Committed dispatchable load, megawatts"
        tabIndex={0}
        onKeyDown={(e) => {
      ...
      >
        ...
        <div
          className="commit-zone right"
          style={{ left: `${(30 / MAX) * 100}%`, width: `${(14 / MAX) * 100}%` }}
        />
        <div className="commit-zone high" style={{ left: `${(44 / MAX) * 100}%`, right: 0 }} />
        <svg
          className="commit-curve"
          viewBox="0 0 960 80"
          preserveAspectRatio="none"
          aria-hidden="true"
        >
          <defs>
            <clipPath id="commit-clip">
              <rect x={0} y={-10} width={pct * 9.6} height={100} />
            </clipPath>
          </defs>
          <path d={curve} />
        ...
        <div
          className="commit-knob"
          style={{
            left: `${pct}%`,
            top: `calc((100% - 22px) * ${((78 - r * 76) / 80).toFixed(4)})`,
          }}
        />
