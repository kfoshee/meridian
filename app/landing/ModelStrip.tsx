...
export default function ModelStrip() {
  ...
  const [tally, setTally] = useState<{
    ...
    week: Window[];
    today: string;
  }>({ hours: 0, events: 0, week: [], today: "" });
  const last = useRef(0);
  const onTally = useCallback((hours: number, events: number, week: Window[], today: string) => {
    const now = performance.now();
    if (now - last.current < 120) return;
    last.current = now; // throttle re-renders
    setTally((t) =>
      t.hours === hours && t.events === events && t.today === today
        ? t
        : { hours, events, week, today },
    );
  }, []);
  useEffect(() => {
  ...
  return (
    <section ref={ref} className={`strip${on ? " in" : ""}`}>
      ...
      <div className="strip-sim">
        <ModelSim onTally={onTally} />
      </div>
