...
export default function Landing() {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const cellPxNow = cellPx(),
        rawPx = ((sy + set * set * (110 - sy)) / 100) * innerHeight;
      const row = rawPx / cellPxNow,
        base = Math.floor(row),
        f = row - base;
      const glide =
        f < 0.3
      ...
      const yc = yearCopy.current;
      if (yc) yc.style.opacity = "1";
      const ys = yp < 0.28 ? -1 : yp < 0.58 ? 0 : yp < 0.8 ? 1 : 2;
      if (ys !== lastY.current) {
        lastY.current = ys;
        setYState(ys);
      }
    ...
    const onMove = (e: MouseEvent) => {
      ...
      if (hc)
        hc.style.transform = `translate(${ox + Math.floor((e.clientX - ox) / cell) * cell}px, ${Math.floor(e.clientY / cell) * cell}px)`;
    };
    ...
    const runPulse = () => {
      const d = pulse.current;
      if (!d) return;
      if (!heroLive()) {
        pulseTimer = window.setTimeout(runPulse, 1500);
        return;
      }
      const cell = cellPx(),
        ox = (vw() / 2) % cell;
      ...
      if (vertical) {
        const x = ox + Math.floor(Math.random() * Math.floor((vw() - ox) / cell)) * cell;
        from = `translate(${x}px, ${dir > 0 ? -110 : innerHeight}px)`;
        to = `translate(${x}px, ${dir > 0 ? innerHeight : -110}px)`;
      } else {
        const y = Math.floor(Math.random() * Math.floor(innerHeight / cell)) * cell;
        from = `translate(${dir > 0 ? -110 : vw()}px, ${y}px)`;
        to = `translate(${dir > 0 ? vw() : -110}px, ${y}px)`;
      }
      anim = d.animate(
        [
        ...
        { duration: 2000, easing: "cubic-bezier(0.4, 0, 0.6, 1)" },
      );
      anim.onfinish = () => {
        pulseTimer = window.setTimeout(runPulse, 3000 + Math.random() * 2500);
      };
    };
    ...
    let cellTimer = 0;
    const tick = () => {
      const pool = cells.current;
      if (!pool) return;
      if (!heroLive()) {
        cellTimer = window.setTimeout(tick, 800);
        return;
      }
      const cell = cellPx(),
        ox = (vw() / 2) % cell;
      const cols = Math.floor((vw() - ox) / cell),
      ...
      const col = nearSun
        ? Math.round(cols / 2 + (Math.random() - 0.5) * 6)
        : Math.floor(Math.random() * cols);
      const row = nearSun
        ? Math.round(sunY / cell + (Math.random() - 0.5) * 5)
        : Math.floor(Math.random() * rows);
      if (col < 0 || col >= cols || row < 0 || row >= rows) {
        cellTimer = window.setTimeout(tick, 120);
        return;
      }
      ...
      d.animate(
        ...
        {
          duration: flash ? 900 : 3000,
          easing: flash ? "cubic-bezier(0.2, 0, 0.4, 1)" : "ease-in-out",
        },
      );
    ...
    <>
      <section ref={scene} className={`hero${entered ? " in" : ""}`}>
        <div className="hero-scene">
          ...
          <div ref={cells} className="hero-fade" aria-hidden="true">
          ...
          <div className="hero-fade" aria-hidden="true">
            <div ref={pulse} className="hero-pulse" />
          </div>
          <div className="hero-sunwrap" aria-hidden="true">
            <div className="hero-sun-tail" />
            <div className="hero-sun-haze" />
            <div className="hero-sun-bloom" />
            <div className="hero-sun-halo" />
            <div className="hero-sun-core" />
          </div>
          <div className="hero-clear hero-fade" />
          <h1 className="hero-copy">
            <strong>Meridian</strong> manages power for AI data centers.
          </h1>
          <div
            ref={yearCopy}
            className={`year-copy${yState >= 0 ? " lit" : ""}`}
            aria-live="polite"
          >
            {yLines.map((l, i) => (
              <p
                key={i}
                className={`year-state${i === yState ? " on" : i < yState ? " past" : ""}`}
              >
                <span>{l}</span>
              </p>
            ))}
      ...
      <footer className="foot" aria-label="Meridian">
        <div className="foot-lockup">
          ...
          <div className="foot-logo-float">
            <NextImage
              className="foot-logo"
              src="/media/meridian-mark-gold.png"
              alt=""
              width={1254}
              height={1254}
            />
