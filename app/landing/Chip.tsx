...
export default function Chip({ tally, on }: { tally: Tally; on: boolean }) {
  const geo = useMemo(() => {
    ...
    const zp = 3;
    const ins = INPUTS.map((_, i) => { const y = -44 + i * 22, L = 34 + (4 - i) * 9; const a = P(-SUB - 7, y, zp), b = P(-SUB - 7 - L, y, zp); return { d: path([[90, b[1]] as const, b, a]), lx: 78, ly: b[1] }; });
    const outs = OUTS.map((_, i) => { const y = -34 + i * 34, L = 24 + i * 16; const a = P(SUB + 7, y, zp), b = P(SUB + 7 + L, y, zp); return { d: path([a, b, [1010, b[1]] as const]), vx: 1024, vy: b[1] }; });
