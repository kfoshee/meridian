...
function seededValues(count: number, seed = 7): number[] {
  let state = seed;
...
export default function Chip({ tally, on }: { tally: Tally; on: boolean }) {
  const geo = useMemo(() => {
    ...
    const random = seededValues(140);
    const vias: (readonly [number, number])[] = [];
    for (let i = 0; i < 70; i++) {
      const gx = -DIE + 6 * Math.floor(random[i * 2] * 12) + 6;
      const gy = -DIE + 6 * Math.floor(random[i * 2 + 1] * 12) + 6;
    ...
    const ins = INPUTS.map((_, i) => {
      ...
      const a = P(-SUB - 7, y, zp),
        b = P(-SUB - 7 - L, y, zp);
      return { d: path([[90, b[1]] as const, b, a]), lx: 78, ly: b[1] };
    });
    const outs = OUTS.map((_, i) => {
      ...
      const a = P(SUB + 7, y, zp),
        b = P(SUB + 7 + L, y, zp);
      return { d: path([a, b, [1010, b[1]] as const]), vx: 1024, vy: b[1] };
    });
  ...
  const nums = [
    String(tally.hours),
    String(tally.events),
    wk ? `${wk.day} ${wk.from}–${wk.to}` : "no window",
  ];

  return (
    <div className={`die-wrap${on ? " on" : ""}${settled ? " settled" : ""}`} tabIndex={-1}>
      <div className="die-row" aria-hidden="true">
        {INPUTS.map((w) => (
          <span key={w}>{w}</span>
        ))}
      </div>
      <svg className="die" viewBox={narrow ? "270 40 640 400" : "0 0 1280 500"} aria-hidden="true">
        <defs>
          ...
          {geo.outs.map((t, i) => (
            <g key={i}>
              ...
              <text className="die-val" x={t.vx} y={t.vy + 10}>
                {nums[i]}
                <tspan className="die-unit" dx={10}>
                  {OUTS[i]}
                </tspan>
              </text>
              <line className="die-rule" x1={t.vx} x2={t.vx + 236} y1={t.vy + 24} y2={t.vy + 24} />
            </g>
          ))}
          <g key={tally.events}>
            {geo.outs.map((t, i) => (
