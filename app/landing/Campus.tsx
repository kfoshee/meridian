...
const L = (a: readonly [number, number], b: readonly [number, number]) => ({
  x1: a[0],
  y1: a[1],
  x2: b[0],
  y2: b[1],
...
function Mark({
  ...
  return (
    <g className={`iso mark${gold ? " gold" : ""}`} style={{ transitionDelay: `${delay}ms` }}>
      ...
      <text x={at[0]} y={at[1] + dy + 3} textAnchor="middle">
        {n}
      </text>
    </g>
  );
...
export const CHAPTERS = [
  { n: 1, name: "Switchyard", spec: "138 kV · 2 × 60 MVA", we: "How much the line can carry." },
  {
    ...
    spec: "the number the grid sees",
    we: "Where the grid sees the drop.",
    gold: true,
  },
  { n: 3, name: "Firm hall", spec: "56 MW · always on", we: "Never turns off." },
  {
    n: 4,
    name: "Flexible hall",
    spec: "40 MW · separable",
    we: "Turns off when the grid is tight.",
    gold: true,
  },
...
export default function Campus({ p }: { p: number }) {
  ...
  const part = (n: number) => ({
    "data-part": n,
    className: `${built(n) ? "on" : ""}${active === n ? " hot" : ""}`,
  ...
  return (
    <div
      className={`campus on${chapter >= 1 ? " started" : ""}${done ? " done" : ""}`}
      data-hot={active ?? undefined}
    >
      <div className="campus-side">
        <div className="chapter" key={active ?? "none"}>
          {active ? (
            <>
              <div className="chapter-n">
                {active} <span>/ 7</span>
              </div>
            ...
            <>
              ...
              <div className="chapter-name">
                {done ? "The whole campus" : "A parcel and a slab"}
              </div>
              {done && (
                <div className="chapter-we">
                  40 of 96 MW can turn off. The rest never does. Hover any part.
                </div>
              )}
            </>
          )}
        ...
        <ol className="legend">
          {CHAPTERS.map((l) => (
            <li
              key={l.n}
              className={`${l.gold ? "gold" : ""}${built(l.n) ? " built" : ""}${active === l.n ? " hot" : ""}`}
              onMouseEnter={() => done && setHover(l.n)}
              onMouseLeave={() => setHover(null)}
            >
              <i>{l.n}</i>
              <div>
      ...
      <svg viewBox="170 -4 860 504" className="campus-svg" aria-hidden="true">
        <g data-part={0} className="on">
          <Box x={0} y={0} w={250} d={132} h={2} tone="dim" delay={0} />
        </g>
        ...
        <g {...part(1)}>
          ...
          <g className="iso" style={{ transitionDelay: "650ms" }}>
            <polygon
              points={pts([P(-52, 30, 0.4), P(-14, 30, 0.4), P(-14, 70, 0.4), P(-52, 70, 0.4)])}
              className="yard"
            />
            {[36, 48, 60].map((y, i) => (
              <line key={i} {...L(P(-50, y, 8), P(-16, y, 8))} className="bus" />
            ))}
          </g>
          {[
            [-34, 33],
        ...
        <g {...part(4)}>
          ...
          <g className="iso" style={{ transitionDelay: "1400ms" }}>
            <line {...L(P(150, 12, 32), P(150, 116, 32))} className="wall" />
          </g>
        ...
        <g {...part(5)}>
          ...
          <g className="iso" style={{ transitionDelay: "700ms" }}>
            {[40, 80].map((y, i) => (
              <polyline key={i} points={pts(pipe(y))} className="pipe" />
            ))}
          </g>
          ...
          <g className="iso" style={{ transitionDelay: "1900ms" }}>
            {Array.from({ length: 9 }, (_, i) => {
              const x = i < 6 ? 36 + i * 19 : 168 + (i - 6) * 22;
              const c = P(x, 104.5, 37);
              return <ellipse key={i} cx={c[0]} cy={c[1]} rx={5} ry={2.6} className="fan" />;
            })}
          </g>
