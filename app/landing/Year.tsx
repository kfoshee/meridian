...
const Year = forwardRef<YearHandle, { photo: HTMLImageElement | null }>(function Year({ photo }, ref) {
  ...
  useImperativeHandle(ref, () => ({
    draw(yp, shrink, card) {
      ...
      const ripple = ease(clamp((yp - 0.30) / 0.34));
      ...
      if (ripple > 0 || dissolve > 0) for (let row = 0; row < rMax; row++) {
        ...
        for (let col = cMin; col < cMax; col++) {
          ...
          if (!STOPSET.has(k)) {
            ...
            if (flash > 0) { g.fillStyle = `rgba(255,246,214,${flash.toFixed(3)})`; g.fillRect(x + gap / 2 - 1, y + gap / 2 - 1, tile + 2, tile + 2); }
      ...
      if (dissolve > 0 && photo) { g.globalAlpha = enter * Math.max(dissolve * 0.85, close); g.drawImage(photo, sx0, sy0, sw, sh); }
