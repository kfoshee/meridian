...
export default function Contact() {
  ...
  useEffect(() => {
    ...
    return () => { document.body.style.overflow = ""; removeEventListener("keydown", onKey); opener.current?.focus(); };
  }, [open]);
  ...
  return (
    <section ref={sec} className={`contact${seen ? " on" : ""}`}>
      <div className="contact-in">
        ...
        <button ref={opener} type="button" className="contact-open" onClick={() => setOpen(true)} aria-haspopup="dialog" aria-expanded={open}>Contact us</button>
      </div>
      ...
      <aside className={`sheet${open ? " open" : ""}`} role="dialog" aria-modal="true" aria-label="Contact Meridian" aria-hidden={!open}>
        ...
        <div className="contact-actions">
          <a href="https://kianfoshee.com" className="contact-open sheet-send" tabIndex={open ? 0 : -1}>Contact Kian ↗</a>
        </div>
      </aside>
    </section>
  );
