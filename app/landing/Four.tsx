...
export default function Four() {
  ...
  useEffect(() => {
    ...
    let raf = 0;
    const update = () => { raf = 0; const el = draftRef.current; if (!el || el.classList.contains("built")) return; const r = el.getBoundingClientRect(); const v = Math.min(1, Math.max(0, -r.top / (r.height - innerHeight))); setDp(d => Math.max(d, v)); /* builds once per page load; scrolling back never un-builds it */
  ...
  return (
    <section ref={ref} className="steps">
      ...
      <article className={`commit${on[0] ? " on" : ""}`} data-io>
        <img className="commit-bg" src="/media/four-commitment.png" alt="" loading="lazy" decoding="async" />
