...
export default function Thesis() {
  ...
  return (
    <section ref={ref} className={`close${on ? " on" : ""}`}><div className="close-in">
      <div className="close-block">
        <p aria-label={TEXT}>{words.map((w, i) => <span key={i} aria-hidden="true" className={KEY.has(w) ? "key" : undefined} style={{ transitionDelay: `${Math.min(i * 28, 2400)}ms` }}>{w}{i < words.length - 1 ? " " : ""}</span>)}</p>
      </div>
    </div></section>
  );
}
